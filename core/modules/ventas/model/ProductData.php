...
class ProductData {
	...
	public function getExistence(){ return OperationData::getQYesF($this->id);}
...
// partiendo de que ya tenemos creado un objecto ProductData previamente utilizamos el contexto
	...
	public static function getAllByPageExistent(&$proexistent,$start_from,$limit,$inicio){
		...
		*/
		$array = array();
		$cnt = 0;
		$objProduct = array_slice($proexistent,$start_from,$limit);
		//while($r = $query[0]->fetch_array()){
		foreach($objProduct as $item=>$data){
			$q=OperationData::getQYesF(intval($data->id));
			if($q>0){
				$array[$cnt] = new ProductData();
				$array[$cnt]->cons = strval($cnt);
				$array[$cnt]->id = $data->id;
				$array[$cnt]->name = $data->name;
				$array[$cnt]->price_in = $data->price_in;
				$array[$cnt]->price_out = $data->price_out;
				$array[$cnt]->image = $data->image;
				$array[$cnt]->unit = $data->unit;
				$array[$cnt]->user_id = $data->user_id;
				$array[$cnt]->category_id = $data->category_id;
				$array[$cnt]->talla_id = $data->talla_id;
				$array[$cnt]->color_id = $data->color_id;
				$array[$cnt]->description = $data->description;
