...
class SellData {
	...
	public static function getSellsByDateSeller($seller,$start,$end){
		$sql = "select * from ".self::$tablename." where seller=$seller and (date(created_at) >= \"$start\" and date(created_at) <= \"$end\") and (operation_type_id=2 or operation_type_id=4 or operation_type_id=5 or operation_type_id=7) order by created_at desc";
		if($start == $end){
		$sql="select * from ".self::$tablename." where date(created_at) = \"$start\" and (operation_type_id=2 or operation_type_id=4 or operation_type_id=5 or operation_type_id=7) order by created_at desc";
		}
		$query = Executor::doit($sql);
	...
	public static function getBySellerId($id){
		$sql = "select * from ".self::$tablename." where seller=$id order by created_at desc";
		$query = Executor::doit($sql);
		$array = array();
		$cnt = 0;
		while($r = $query[0]->fetch_array()){
