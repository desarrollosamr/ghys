...
class OperationData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (product_id,ajuste_id,q,d,operation_type_id,sell_id,created_at) ";
		$sql .= "values ($this->product_id,$this->ajuste_id,$this->q,$this->d,$this->operation_type_id,$this->sell_id,$this->created_at)";
...
////////////////////////////////////////////////////////////////////
	...
	public static function getQYesF($product_id){
		...
		$output_id = OperationTypeData::getByName("salida")->id;
		foreach($operations as $operation){
			if($operation->operation_type_id==$input_id or $operation->operation_type_id==3 or $operation->operation_type_id==8){ $q+=$operation->q; }
			else if($operation->operation_type_id==$output_id or $operation->operation_type_id==4 or $operation->operation_type_id==5 or $operation->operation_type_id==9){  $q+=(-$operation->q); }
		}
		// print_r($data);
		return $q;
	}
